...
if mode == 'lower-tail':
    ...
    p = float(input('Please Enter the percentile: '))
    b2 = scipy.stats.norm.ppf(p / 100, mu, sigma)
    ...
    print(mode, 'is an incorrect mode. It must be lower-tail, upper-tail, or interval.')
    exit(0)

# region = np.linspace(b1, b2, 100)
# plt.fill_between(region, stats.norm.pdf(region, mu, sigma), color='pink')

plt.savefig('NormalSketch.png', format='png', dpi=1200)
